...
namespace {
constexpr float kDailyFineAmount = 1.0f;
}
...
void LoansCollection::AutoUpdateLoanStatus() {
    ...
    for (auto* loan : loansList) {
        if (calculateDaysDifference(loan->getDueDate(), today) < 0) {
            ...
            loan->setStatus(Loans::NORMAL);
        }
    }
}

void LoansCollection::RecalculatePatronFines(PatronsCollection &allPatrons) {
    allPatrons.ResetAllFines();

    std::tm today = getCurrentDate();
    for (auto* loan : loansList) {
        int daysOverdue = -calculateDaysDifference(loan->getDueDate(), today);
        if (daysOverdue > 0) {
            allPatrons.AddFineToPatron(loan->getPatronID(), daysOverdue * kDailyFineAmount);
        }
    }
}

void LoansCollection::MarkLoanAsOverdueForTesting(PatronsCollection &allPatrons, BooksCollection &allBooks) {
    Patron* patron = allPatrons.PromptForSearchMechanism();
    if (!patron) {
        std::cout << "Patron not found.\n";
        return;
    ...
    if (it == loansList.end()) {
        std::cout << "No active loan found for this patron/book.\n";
        return;
    }

    std::tm overdueDate = getCurrentDate();
    overdueDate.tm_mday -= 3;
    std::mktime(&overdueDate);

    (*it)->setDueDate(overdueDate);
    (*it)->setStatus(Loans::OVERDUE);
    RecalculatePatronFines(allPatrons);

    std::cout << "Loan marked overdue for testing. New due date: " << tmToString(overdueDate) << ".\n";
}
