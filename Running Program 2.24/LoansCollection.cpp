...
/**
 * Marks an active loan as overdue for testing purposes
 * Prompts for patron and book, then backdates due date by 8 days
 *
 * @param allPatrons - Reference to PatronsCollection for patron lookup
...
void LoansCollection::MarkLoanAsOverdueForTesting(PatronsCollection& allPatrons, BooksCollection& allBooks) {
    ...
    if (loansList.empty()) {
        std::cout << "No active loans to mark as overdue.\n";
        return;
    }
    ...
    auto it = std::find_if(loansList.begin(), loansList.end(), [&](Loans* loan) {
        return loan->getPatronID() == patron->getPatronID()
            && loan->getBookID() == book->getLibraryID();
        });

    if (it == loansList.end()) {
        std::cout << "Loan record not found for this patron and book.\n";
        return;
    }

    std::tm overdueDueDate = getCurrentDate();
    overdueDueDate.tm_mday -= 8;
    std::mktime(&overdueDueDate);

    (*it)->setDueDate(overdueDueDate);
    (*it)->setStatus(Loans::OVERDUE);

    std::cout << "Loan marked as overdue for testing (8 days overdue).\n";
}
