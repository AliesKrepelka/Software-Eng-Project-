...
int calculateDaysDifference(const std::tm& dueDate, const std::tm& currentDate) {   
    ...
    const double secondsPerDay = 86400.0; // 60 sec * 60 min *24 hrs
    double days = std::difftime(due_time, current_time) / secondsPerDay;
    return static_cast<int>(std::floor(days + 1e-9));
}
//Didn't touch any of this code below, just wanted to say only the above code was added to.
...
void LoansCollection::CheckInBook(PatronsCollection &allPatrons, BooksCollection &allBooks) {
    ...
    if (it != loansList.end()) {
        ...
        if (daysDiff < 0) {
            ...
            double newBal = std::round((patron->getFineBalance() + fine) * 100.0) / 100.0; // cents
            patron->setFineBalance(static_cast<float>(newBal)); // 2 decimal places
            int fineCents = static_cast<int>(std::round(fine * 100));
            int fineWhole = fineCents / 100;
            int fineFrac = fineCents % 100;
            std::cout << "Book overdue by " << daysLate << " days. Fine $" 
                      << fineWhole << "." << (fineFrac < 10 ? "0" : "") << fineFrac 
                      << " applied. New balance: $" << newBal << "\n";
